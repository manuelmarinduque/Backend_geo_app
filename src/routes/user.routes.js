...
router.put('/update_user/:doc_number', (req, res) => {
    ...
    mysqlConnection.query('UPDATE user SET doc_number=?, doc_type=?, full_name=?, password=?, genre=?, nacionality=?, address=?, phone=? WHERE doc_number=? AND is_active=1',
...
router.delete('/delete_user/:doc_number', (req, res) => {
    mysqlConnection.query(`UPDATE user SET is_active=1 WHERE doc_number = ${req.params.doc_number}`, (error, row) => {
        error ? res.status(500).json({message: error.message}) : res.status(200).json({data: row})
    })
})
